...
export class ReservationPrismaRepository implements AddReservationRepository, CancelReservationRepository, LoadReservationByIdRepository, FinishReservationRepository, LoadReservationsRepository, SendReservationInviteRepository {
  async send(inviteData: SendReservationInviteModel): Promise<ReservationInviteModel[] | null> {
    const guests = await Promise.all(inviteData.guests.map((guest) => prismaClient.guest.create({ data: guest })));

    await prismaClient.reservationGuest
      .createMany({ data: guests.map((guest) => ({ reservationId: inviteData.reservationId, guestId: guest.id })) });

    return guests;
  }

  async load({ accountId }: LoadReservationsModel): Promise<ReservationModel[] | null> {
    const reservations = await prismaClient.reservation.findMany({ where: { accountId: accountId }, include: { feedback: true } });
  ...
  async loadById(id: string): Promise<ReservationModel | null> {
    const reservation = await prismaClient.reservation.findFirst({ where: { id }, include: { feedback: true, space: true, guests: true } });
    ...
    return reservation && {
      ...
      space: {
        ...
        complement: reservation.space.complement === null ? undefined : reservation.space.complement,
      },
    };
  }
